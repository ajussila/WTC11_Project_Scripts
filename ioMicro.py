...
class get_dapi_features:
    def __init__(self,fl,save_folder,set_='',gpu=True,im_med_fl = r'C:\Users\cfg001\Desktop\WTC11\flat_field\Scope4_med_col_raw3.npz',
                psf_fl = r'C:\Users\cfg001\Desktop\WTC11\psfs\psf_750_Scope4_final.npy',redo=False):
    ...
    def load_im(self):
        ...
        imD = full_deconv(im,psf=self.psf,gpu=self.gpu,force=True,parameters={'method': 'wiener', 'beta': 0.01, 'niter': 50})
...
def get_stats_D_H_cor(dec,icol=0,plt_val=False,dic_th={'cor':[0.6,0.6],'H':[9,9],'D':[1.25,1.25]}):
    ...
    return keep
def get_best_translation_pointsV2(fl,fl_ref,save_folder,set_,resc=5,th=5):
...
def get_intersV2(self,nmin_bits=4,dinstance_th=2,enforce_color=True,enforce_set=17*3,redo=False):
    """Get an initial intersection of points and save in self.res"""
    self.res_fl = self.decoded_fl.replace('decoded','res')
    if not os.path.exists(self.res_fl) or redo:
...
def get_score_withRef(dec,scoresRef,plt_val=False,gene=None,iSs=None,th_min=-np.inf,include_dbits=False):
    ...
    for icol in range(dec.ncols):
        ...
        if iSs is None: iSs = np.arange(scoresRef_.shape[-1])
        for iS in iSs:
            kdt = KDTree(scoresRef_[:,[iS]])
            dist_,inds_ = kdt.query(score_[:,[iS]],workers=20)
            scoreA_+=np.log((inds_+1))-np.log(len(scoresRef_))
        scoreA[keep_color[icol]]=scoreA_
    dec.scoreA =scoreA
    if plt_val:
...
def keep_best_per_cell_fast(XH_f,nbest=20):
    ...
    return XH_f_pruned
def get_best_translation_points_dapiFeat(fl,fl_ref,resc=5,th=5):
    obj_Xh_plus = np.load(fl)['Xh_plus']
    obj_Xh_minus = np.load(fl)['Xh_minus']
    obj_ref_Xh_plus = np.load(fl_ref)['Xh_plus']
    obj_ref_Xh_minus = np.load(fl_ref)['Xh_minus']
    tzxyf,tzxy_plus,tzxy_minus,N_plus,N_minus = np.array([0,0,0]),np.array([0,0,0]),np.array([0,0,0]),0,0
    if (len(obj_Xh_plus)>0) and (len(obj_ref_Xh_plus)>0):
        
        X = obj_Xh_plus
        X_ref = obj_ref_Xh_plus
        X = X[X[:,-1]>th][:,:3]
        X_ref = X_ref[X_ref[:,-1]>th][:,:3]
        tzxy_plus,N_plus = get_best_translation_points(X,X_ref,resc=resc,return_counts=True)
    if (len(obj_Xh_minus)>0) and (len(obj_ref_Xh_minus)>0):
        X = obj_Xh_minus
        X_ref = obj_ref_Xh_minus
        X = X[X[:,-1]>th][:,:3]
        X_ref = X_ref[X_ref[:,-1]>th][:,:3]
        tzxy_minus,N_minus = get_best_translation_points(X,X_ref,resc=resc,return_counts=True)
    if np.max(np.abs(tzxy_minus-tzxy_plus))<=2:
